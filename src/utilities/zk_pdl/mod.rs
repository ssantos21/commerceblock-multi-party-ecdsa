...
use curv::arithmetic::{traits::Samplable, big_num::Zero};
...
impl Verifier {
    pub fn message1(statement: &PDLStatement) -> (PDLVerifierFirstMessage, PDLVerifierState) {
        ...
        let q = FE::q();
        let q_sq = q.checked_mul(&q).unwrap();
        ...
        let ab_concat = a.clone() + b.clone().shl(a.bits());
    ...
    pub fn message2(
        ...
        let ab_concat = &verifier_second_message.a
            + verifier_second_message
                .b
                .clone()
                .shl(verifier_second_message.a.bits()); // b|a (in the paper it is a|b)
